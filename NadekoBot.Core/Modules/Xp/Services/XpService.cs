        ...
        public Task<(Stream Image, IImageFormat Format)> GenerateXpImageAsync(FullUserStats stats) => Task.Run(async () =>
        {
            using (var img = Image.Load(_images.XpBackground, out var imageFormat))
            {
                if (_template.User.Name.Show)
                {
                    ...
                    username = username.Length > 18 ? username.Substring(0,15) + "..." : username;
                    var usernameFont = _fonts.NotoSans.CreateFont(_template.User.Name.FontSize, FontStyle.Bold);
                    img.Mutate (x => {
                        x.DrawText (new TextGraphicsOptions { HorizontalAlignment = HorizontalAlignment.Left, VerticalAlignment = VerticalAlignment.Center }, username, usernameFont,
                ...
                if (_template.Club.Name.Show)
                {
                    ...
                    clubName = clubName.Length > 18 ? clubName.Substring(0,15) + "..." : clubName;

                    var clubFont = _fonts.NotoSans.CreateFont(_template.Club.Name.FontSize, FontStyle.Bold);
                    img.Mutate (x => x.DrawText (new TextGraphicsOptions { HorizontalAlignment = HorizontalAlignment.Right, VerticalAlignment = VerticalAlignment.Center }, clubName, clubFont,
                        _template.Club.Name.Color,
                        new PointF (_template.Club.Name.Pos.X, _template.Club.Name.Pos.Y)));
                }
                ...
                if (_template.User.Xp.Bar.Show)
                {
                 ...
                 if (_template.User.Xp.Global.Show) {
                    img.Mutate (x => x.DrawText ($"{global.LevelXp}/{global.RequiredXp}",
                        ...
                        _template.User.Xp.Global.Color,
                ...
                if (stats.FullGuildStats.AwardedXp != 0 && _template.User.Xp.Awarded.Show)
                {
                    ...
                    img.Mutate (x => x.DrawText ($"({sign}{stats.FullGuildStats.AwardedXp})",
                        ...
                        _template.User.Xp.Awarded.Color,
                        new PointF (awX, awY)));
                }
                ...
                if (_template.User.GlobalRank.Show)
                {
                    img.Mutate(x => x.DrawText(stats.GlobalRanking.ToString(),
                        _fonts.NotoSans.CreateFont(_template.User.GlobalRank.FontSize, FontStyle.Bold),
